...
export function buildVehicleLights(exterior, anchors, car, game) {
  ...
  for (const x of [-0.62, 0.62]) {
    const s = new THREE.SpotLight(0xfff2dc, 0, 90, 0.6, 0.35, 2);
    s.position.set(x, 0.78, -2.2); s.castShadow = false;
    s.target.position.set(x * 0.6, -0.4, -30); root.add(s); root.add(s.target);
    heads.push(s);
  }
  const brake = new THREE.PointLight(0xff2010, 0, 5, 2); brake.position.set(0, 0.7, 3.6); root.add(brake);
  const reverse = new THREE.PointLight(0xffffff, 0, 6, 2); reverse.position.set(0, 0.7, 3.7); root.add(reverse);
  ...
  function update(dt, night) {
    ...
    for (const h of heads) { h.intensity = lerp(0, highOn ? 16000 : 7500, Math.min(1, S.headLevel * 2)); h.angle = highOn ? 0.5 : 0.62; h.distance = highOn ? 200 : 110; h.visible = h.intensity > 1; }
    heads[0].target.position.set(-0.3, highOn ? -0.6 : -1.6, highOn ? -50 : -22); heads[1].target.position.set(0.3, highOn ? -0.6 : -1.6, highOn ? -50 : -22);
    ...
    brake.intensity = on && L.brake ? 10 : 0; brake.visible = brake.intensity > 0;
    const rev = on && L.reverse;
    lamps.revL.material.emissiveIntensity = rev ? 3 : 0; reverse.intensity = rev ? 14 : 0; reverse.visible = rev;
