...
export function makeMaterials(T) {
  ...
  const M = {
    ...
    glass: std({ color: 0xcfe3ea, transparent: true, opacity: 0.08, roughness: 0.25, metalness: 0, depthWrite: false, side: THREE.DoubleSide, envMapIntensity: 0.12 }),
    glassTint: std({ color: 0x3a5560, transparent: true, opacity: 0.35, roughness: 0.3, depthWrite: false, side: THREE.DoubleSide, envMapIntensity: 0.1 }),
    needle: std({ color: 0xff2a1a, emissive: 0xff2a1a, emissiveIntensity: 1.6, roughness: 0.5 }),
    paint: std({ color: 0x8f9cad, roughness: 0.42, metalness: 0.5, envMapIntensity: 0.8 }),
